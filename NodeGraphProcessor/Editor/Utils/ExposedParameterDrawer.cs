...
namespace GraphProcessor
{
    ...
    public class Vector4ParameterDrawer : ExposedParameterDrawer
    {
        public override VisualElement CreatePropertyGUI(SerializedProperty property)
        {
            var container = new VisualElement();
            ...
            // 使用 UI Toolkit 的 Vector4Field，而非 IMGUIContainer + EditorGUILayout。
            // IMGUI 嵌在 UI Toolkit 里在浮动窗口/未打开窗口时拿不到稳定的 Layout/Repaint 事件，
            // 四分量复合字段会因此无法正常接收输入（表现为"不可修改"）。
            var field = new Vector4Field(name.stringValue) { value = val.vector4Value };
            field.RegisterValueChangedCallback(e => {
                val.vector4Value = e.newValue;
                ApplyModifiedProperties(property);
            });
