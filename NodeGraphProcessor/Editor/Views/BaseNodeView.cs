...
namespace GraphProcessor
{
	...
	public class BaseNodeView : NodeView
	{
        ...
        /// <summary>悬浮于节点顶部外侧的 rid 标签（绝对定位，不撑大节点）</summary>
        Label										ridLabel;
        /// <summary>图数据变化时刷新 rid 标签的回调（增删节点/撤销时 Rid 序号可能变化）</summary>
        Action< GraphChanges >						onGraphChangedHandler;
        ...
        /// <summary>创建悬浮于节点顶部的 rid 标签，并订阅图数据变化以保持序号一致</summary>
        void InitializeRidLabel()
        {
            ridLabel = new Label()
            {
                pickingMode = PickingMode.Ignore,
                tooltip = "节点 Rid（对应运行时日志）",
            };
            ridLabel.style.position = Position.Absolute;      // 绝对定位，脱离文档流，不撑大节点宽度
            ridLabel.style.top = -16;                          // 悬浮于节点顶部外侧
            ridLabel.style.left = 0;
            ridLabel.style.right = 0;                          // 横跨节点宽度，便于文本右对齐
            ridLabel.style.unityTextAlign = TextAnchor.MiddleRight;
            ridLabel.style.fontSize = 10;
            ridLabel.style.paddingLeft = 4;
            ridLabel.style.paddingRight = 4;
            ridLabel.style.color = new Color(0.75f, 0.75f, 0.78f, 1f);
            Add(ridLabel);

            onGraphChangedHandler = _ => RefreshRidLabel();
            if (owner != null && owner.graph != null)
                owner.graph.onGraphChanges += onGraphChangedHandler;
            RefreshRidLabel();
        }

        /// <summary>刷新 rid 文本：与导出转换一致，rid = 1000 + 节点在 graph.nodes 中的序号；受节点表 SO 开关控制</summary>
        public void RefreshRidLabel()
        {
            if (ridLabel == null || owner == null || owner.graph == null)
                return;
            if (!owner.graph.showNodeRidLabel)
            {
                ridLabel.visible = false;
                return;
            }
            int idx = owner.graph.nodes.IndexOf(nodeTarget);
            if (idx < 0)
            {
                ridLabel.visible = false;
                return;
            ...
            ridLabel.text = (1000 + idx).ToString();
        }

        /// <summary>取消图数据变化订阅，避免节点视图销毁后仍被回调</summary>
        void UnsubscribeGraphChanges()
        {
            if (onGraphChangedHandler != null && owner != null && owner.graph != null)
                owner.graph.onGraphChanges -= onGraphChangedHandler;
            onGraphChangedHandler = null;
        }
		...
		void InitializeView()
		{
			...
			// 始终初始化重命名编辑器（隐藏的 TextField），右键菜单可触发
		SetupRenamableTitle();
		}
	...
	void SetupRenamableTitle()
	{
		...
		// 仅在 isRenamable 时允许双击标题触发编辑
		if (nodeTarget.isRenamable)
		{
	...
	/// <summary>
	/// 通过右键菜单或程序调用打开节点重命名编辑器
	/// </summary>
	public void BeginRename()
	{
		if (titleTextField == null || titleLabel == null)
			return;
	...
	void CloseAndSaveTitleEditor(string newTitle)
	{
		...
		internal void OnPortDisconnected(PortView port)
		{
			...
			// 多连端口：仅当该字段所有端口都不再有连接时才显示输入框；
			// 删除一条但仍有其他连接时保持隐藏，避免多连场景误弹编辑框。
			if (hideElementIfConnected.TryGetValue(port.fieldName, out var elem))
			{
				bool anyConnected = portsPerFieldName.TryGetValue(port.fieldName, out var pvs)
								  && pvs.Any(pv => pv.GetEdges().Count > 0);
				elem.style.display = anyConnected ? DisplayStyle.None : DisplayStyle.Flex;
			}
