...
namespace GraphProcessor
{
	...
	public class BaseGraphView : GraphView, IDisposable
	{
        ...
        /// <summary>
        /// 整图重载（撤销/重做 ReloadView、ClearGraphElements）期间进入"拆除视图"阶段。
        /// 此时元素从 GroupView 移出只是视图重建，不应据此改写 group.innerNodeGUIDs，
        /// 否则会把已被撤销/恢复的组数据写空（见 GroupView.OnElementsRemoved）。
        /// </summary>
...
#pragma warning restore CS0612 // 类型或成员已过时
		...
		protected virtual NodeInspectorObject CreateNodeInspectorObject()
		{
			...
			inspector.name = "数据 Node Inspector";
		...
		{
            get { return selection.Any(e => e is BaseNodeView || e is GroupView || e is StickyNoteView); }
		}
		...
		void UnserializeAndPasteCallback(string operationName, string serializedData)
		{
			...
			foreach (var serializedNode in data.copiedNodes)
			{
				...
				node.OnNodeCreated();
				// OnNodeCreated 会初始化节点，但也可能重置业务字段（如 NDebugLog.text 被置空）。
				// 把反序列化回来的字段覆盖回节点以保留参数，再恢复 OnNodeCreated 重生的新 GUID，避免与源节点冲突。
				string regeneratedGUID = node.GUID;
				EditorJsonUtility.FromJsonOverwrite(serializedNode.jsonDatas, node);
				node.GUID = regeneratedGUID;
            ...
            // 重建便签（Sticky Note）：位置微偏移避免与原便签重叠
            foreach (var serializedSticky in data.copiedStickyNotes)
			{
				var note = JsonSerializer.Deserialize< StickyNote >(serializedSticky);
				note.position.position += new Vector2(20, 20);
				var noteView = AddStickyNote(note);
		...
		public override List< Port > GetCompatiblePorts(Port startPort, NodeAdapter nodeAdapter)
		{
			...
			compatiblePorts.AddRange(ports.ToList().Where(p => {
				...
				// Relay 节点的 object 端口作为万能接头，兼容一切类型（包括 exec）
				var startOwner = (startPort as PortView)?.owner;
				var targetOwner = (p as PortView)?.owner;
				bool isRelayPort = startOwner?.nodeTarget is RelayNode
				                || targetOwner?.nodeTarget is RelayNode;

				if (!isRelayPort && !BaseGraph.TypesAreConnectable(startPort.portType, p.portType))
					return false;
		...
		protected virtual void BuildGroupContextualMenu(ContextualMenuPopulateEvent evt, int menuPosition = -1)
		{
            ...
            evt.menu.InsertAction(menuPosition, "创建组 Create Group", (e) => AddSelectionsToGroup(AddGroup(new Group("组 Group", position))), DropdownMenuAction.AlwaysEnabled);
		}
...
#if UNITY_2020_1_OR_NEWER
			Vector2 position = (evt.currentTarget as VisualElement).ChangeCoordinatesTo(contentViewContainer, evt.localMousePosition);
            evt.menu.InsertAction(menuPosition, "创建便签 Create Sticky Note", (e) => AddStickyNote(new StickyNote("便签 Sticky Note", position)), DropdownMenuAction.AlwaysEnabled);
#endif
		}

		/// <summary>
		/// Add the Rename entry to the context menu
		/// </summary>
		protected virtual void BuildRenameContextualMenu(ContextualMenuPopulateEvent evt)
		{
			var selectedNodeViews = selection.Where(s => s is BaseNodeView).Cast< BaseNodeView >().ToList();

			evt.menu.AppendAction("重命名 Rename",
				(e) => {
					if (selectedNodeViews.Count > 0)
						selectedNodeViews[0].BeginRename();
				},
				(e) => selectedNodeViews.Count == 1 ? Status.Normal : Status.Disabled
			);
		}
		...
		{
			evt.menu.AppendAction("高亮文件 Select Asset", (e) => EditorGUIUtility.PingObject(graph), DropdownMenuAction.AlwaysEnabled);
		}
		...
		{
			evt.menu.AppendAction("保存文件 Save Asset", (e) => {
...
#pragma warning restore CS0612 // 类型或成员已过时
		...
		/// <summary>刷新画布中所有节点的 Rid 标签可见性/文本（节点表 SO 的开关变化时调用）</summary>
		public void RefreshRidLabelsVisibility()
		{
			foreach (var view in nodeViews)
				view.RefreshRidLabel();
		}
		...
		/// </summary>
		public void DelayedDeleteSelection() => this.schedule.Execute(() => DeleteSelectionOperation("删除 Delete", AskUser.DontAskUser)).ExecuteLater(0);
