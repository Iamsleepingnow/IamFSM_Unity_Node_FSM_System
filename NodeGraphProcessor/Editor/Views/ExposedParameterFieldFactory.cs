...
namespace GraphProcessor
{
    ...
    public class ExposedParameterFieldFactory : IDisposable
    {
        ...
        {
            RefreshSerializedParameters();
            int propIndex = FindPropertyIndex(parameter);
            if (propIndex < 0)
            {
                Debug.LogWarning($"[ExposedParameterFieldFactory] 找不到参数 {parameter?.name}，可能在列表重建期间被移除");
        ...
        // 每次获取字段前都重新绑定 serializedParameters：参数列表在增删后会发生结构性变化，
        // 缓存的 SerializedProperty 句柄会失效（索引错位），必须重新 FindProperty 才能安全访问。
        void RefreshSerializedParameters()
        {
            serializedObject.Update();
            serializedParameters = serializedObject.FindProperty(nameof(ExposedParameterWorkaround.parameters));
        }
