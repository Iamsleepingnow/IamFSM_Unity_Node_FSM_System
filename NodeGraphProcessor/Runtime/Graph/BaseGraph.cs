...
namespace GraphProcessor
{
	...
	public class BaseGraph : ScriptableObject, ISerializationCallbackReceiver
	{
		...
		/// <summary>是否在画布节点顶部显示 Rid 标签（默认关闭，随节点表 SO 持久化）</summary>
		[SerializeField]
		public bool										showNodeRidLabel = false;
        ...
        protected virtual void OnEnable()
        {
			...
			// 始终重建数据端口：端口（NodePort）与边的引用均为非序列化数据，
			// 域名重载（编译）后 node 为全新实例，端口必须经 InitializeGraphElements 重建。
			// Play 模式仅跳过"删边/计算序"，避免 Play 进出时的序列化竞态误删边；
			// 不能再跳过端口重建，否则"运行编辑器时触发编译"会导致端口永久消失。
		...
		void InitializeGraphElements()
		{
			...
			foreach (var edge in edges.ToList())
			{
				...
				{
					// 端口未解析 → 跳过不删除。可能是过渡状态（节点引用未就绪），
					// 下次序列化/反序列化周期中 GUID 恢复后可重新连接。
					continue;
				}
		...
		{
			isEnabled = false;
			// Play 模式下不清理端口：端口随 ScriptableObject 序列化保留，
			// 避免 清空→重建 循环中端口断连
			if (!Application.isPlaying)
			{
		...
		{
			// 只删除"真孤儿"边：空端口字段名（数据已损坏），或引用的节点已彻底不在图中。
			// 若 inputNode/outputNode 为 null 但对应 nodeGUID 仍存在于图中，说明是重载竞态下
			// Deserialize() 暂时未锁定节点 — 此时保留边，下次序列化周期会自动重连，
			// 避免把一条健康的执行边（Executes→Executed）在瞬时窗口内误删成永久断连。
			var toRemove = edges.Where(e =>
				string.IsNullOrEmpty(e.inputFieldName) || string.IsNullOrEmpty(e.outputFieldName)
				|| (e.inputNode == null && !nodes.Any(n => n != null && n.GUID == e.inputNodeGUID))
				|| (e.outputNode == null && !nodes.Any(n => n != null && n.GUID == e.outputNodeGUID))
			).ToList();
			...
			if (toRemove.Count > 0)
			{
				...
				{
					Debug.LogWarning($"[BaseGraph] 移除孤儿边 {e.outputFieldName}->{e.inputFieldName} " +
						$"(inNodeGUID={e.inputNodeGUID}, outNodeGUID={e.outputNodeGUID}, 边GUID={e.GUID})。若此边本应存在，请勿直接删除数据，联系排查重载竞态。");
				}
				foreach (var e in toRemove)
					edges.Remove(e);
			}
		...
		public static bool TypesAreConnectable(Type t1, Type t2)
		{
			...
			// 执行端口（ConditionalLink）只能与执行端口互连，不能连接数据端口
			if (t1 == typeof(ConditionalLink) != (t2 == typeof(ConditionalLink)))
				return false;
