...
// We keep slow checks inside the editor
				...
				{
					Debug.LogError("Can't convert from " + outputField.FieldType + " to " + inputField.FieldType + ", you must specify a custom port function (i.e CustomPortInput or CustomPortOutput) for non-implicit convertions");
					return null;
				}
#endif
				...
				{
					// 字段为 object 时，先按端口 displayType(outType) 拆箱，再转换到目标类型，
					// 避免 object 直接拆箱到 inputField.FieldType 抛 InvalidCastException（如 float 参数连 int 输入）。
					var castExpr = outputParamField;
					if (outputField.FieldType == typeof(object) && outType != typeof(object))
						castExpr = Expression.Convert(castExpr, outType);
					outputParamField = Expression.Convert(castExpr, inputField.FieldType);
				}
